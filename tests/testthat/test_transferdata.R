...
test_that("TransferData default work", {
  ...
  expect_equal(preds.standard[1, 5], 0.4273605371, tolerance = 1e-6)
  ...
  expect_equal(sum(GetAssayData(pred.assay)[1, ]), 64.64129231, tolerance = 1e-6)
  expect_equal(sum(GetAssayData(pred.assay)[, 1]), 281.090076, tolerance = 1e-6)
...
test_that("TransferData can return predictions assay, ", {
  ...
  expect_equal(pred.assay@var.features, logical(0))
  expect_equal(ncol(pred.assay@meta.features), 0)
  expect_equal(sum(GetAssayData(pred.assay)[1, ]), 26.52830352, tolerance = 1e-6)
  expect_equal(sum(GetAssayData(pred.assay)[, 1]), 1.427360537, tolerance = 1e-6)
...
test_that("TransferData handles weight.reduction properly, ", {
  ...
  expect_equal(sum(pca.preds[, 2]), 27.70926483, tolerance = 1e-6)
