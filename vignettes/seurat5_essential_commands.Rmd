---
title: "Seurat v5 Essential Commands"
output:
  html_document:
    theme: united
    df_print: kable
...
Get cell names. Since Seurat v5 objects don't require all assays to have the same cells, `Cells()` is designed to get cell names of the default assay and `colnames()` is deigned to get cell names across the entire object.
...
By setting a global option (`Seurat.object.assay.version`), you can default to creating either Seurat v3 assays, or Seurat v5 assays. The use of v5 assays is set by default upon package loading, which ensures backwards compatibility with existing workflows.
...
`CreateAssayObject()` and `CreateAssay5Object()` can be used to create v3 and v5 assays respectively, regardless of the setting in `Seurat.object.assay.version`.
...
VariableFeatures(pbmc3k)
# returns information from the assay, cell embeddings, and meta.data as a data.frame
