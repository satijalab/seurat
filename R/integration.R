...
#' anchors are used in the \code{\link{MapQueryData}} function
#' 
#' @param normalization.method Name of normalization method used on the reference: 
#' LogNormalize or SCT;  the same method should be applied to the query
#' @param reference.neighbors Name of the Neighbor to use from the reference (generated by NNHelper)
#' @param reference.reduction Dimensional reduction to use from the reference
#' @param reduction Name for the projected reduction in the query object
#' @inheritParams FindTransferAnchors
#' 
#' @export
#' 
FindMappingAnchors <- function(
  reference,
  query,
  normalization.method = c("LogNormalize", "SCT"),
  ...
  if (is.null(reference.neighbors)) {
    stop("reference.neighbors must be provided. Use NNHelper to generate the Neighbor object.")
  }
  if (!(reference.neighbors %in% Neighbors(reference))) {
    stop("reference.neighbors not found in Neighbor slot of the reference. Use NNHelper to generate the Neighbor object.")
  }
  projected = TRUE
  normalization.method <- match.arg(arg = normalization.method)
...
MapQueryData <- function(reference,
  ...
  objects.list <- lapply(
    ...
    FUN = function(i) {
      ...
      merged.obj[["query_ref.nn"]] <- query_ref.nn
      return (merged.obj)
    }
  )
