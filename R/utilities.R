...
#' (\code{\link[Matrix:dgCMatrix-class]{dgCMatrix}}). The result is fully self-contained, so a
#' plain \code{\link[base]{saveRDS}} writes it to a single portable file with no
#' external on-disk matrix directories to keep alongside it. Use when the data
#' fits in memory; for layers exceeding the \code{dgCMatrix} \eqn{2^{31}} limit,
#' keep the on-disk backend and use \code{\link{SaveSeurat}} instead.
#'
#' @param object A \code{\link[SeuratObject]{Seurat}} object
...
#'   \code{dgCMatrix}
#'
#' @seealso \code{\link{SaveSeurat}}, \code{\link{LoadSeurat}}
#'
#' @export
#' @concept utilities
#'
#' @examples
#' \dontrun{
#' # obj has BPCells layers on disk
#' obj <- AsInMemory(obj)
...
AsInMemory <- function(object, assays = NULL, layers = NULL, verbose = TRUE) {
  ...
  for (assay in assays) {
    ...
    assay.layers <- intersect(
      x = assay.layers,
      y = Layers(object = object, assay = assay)
    )
      ...
      LayerData(object = object, layer = lyr, assay = assay) <- tryCatch(
        ...
        error = function(e) {
          stop(
            ...
            'on-disk backend and use SaveSeurat() to write a single portable ',
            'file. Original error: ', conditionMessage(e),
            call. = FALSE
          )
        }
      )
