...
#' Read Akoya CODEX data
#'
#' Read Akoya CODEX data
#'
#' @param matrix Path to matrix generated by upstream processing.
#' @param file.type Specify which type matrix is being provided.
#' \itemize{
...
ReadAkoya <- function(
  ...
  # Check arguments
  if (!file.exists(matrix) || dir.exists(matrix)) {
    stop(paste("Can't file file:", matrix))
  }
  mtx.file.type <- match.arg(
    arg = file.type,
    choices = c("inForm", "processor", "QuPath")
  )
  type <- "centroids"

  outs <- list("matrix"=NULL, "centroids"=NULL)

  if (mtx.file.type == "inForm") {
    sep = "\t"
  } else {
    sep = ","
  }
  ...
  if (mtx.file.type == "processor") {
    outs[["centroids"]] <- data.frame(
      x = mtx[["x:x"]],
      y = mtx[["y:y"]],
      cell = mtx[["cell_id:cell_id"]],
      stringsAsFactors = FALSE
    )

    # drop all DAPI, Blank, and Empty columns from the matrix
    for (col in colnames(mtx)) {
      # get rid of columns we don't want as metadata or in the final counts matrix
      if (grepl("DAPI", col) || grepl("Blank", col) || grepl("Empty", col)) {
        mtx[[col]] <- NULL
      }
    }

    bcs <- as.character(mtx[["cell_id:cell_id"]])
    rownames(x = mtx) <- bcs

    # filter out remaining non-feature columns
    mtx <- mtx[,grepl("cyc", colnames(mtx))]
    c <- c()
    for (col in colnames(mtx)) {
      c <- c(c, strsplit(col, ":")[[1]][2])
    }
    colnames(mtx) <- c
    mtx <- as.data.frame(t(x = as.matrix(x = mtx[, -1, drop = FALSE])))

    outs[["matrix"]] <- mtx
  } else if (mtx.file.type == "inForm") {
    # print(colnames(mtx))
    feature.cols <- colnames(mtx)[grepl("^Entire cell(.*)Mean(.*)", colnames(mtx), ignore.case=T)]
    
    # Example feature column name: "Entire Cell DAPI-01 (DAPI-01) Mean (Normalized Counts, Total Weighting)"
    feature.names <- lapply(feature.cols, FUN = function(x) strsplit(x, " ")[[1]][3])
    outs[["matrix"]] <- t(mtx[, feature.cols])
    rownames(outs[["matrix"]]) <- feature.names
    colnames(outs[["matrix"]]) <- as.character(mtx[["Cell ID"]])
    outs[["centroids"]] <- data.frame(
      x = mtx[["Cell X Position"]],
      y = mtx[["Cell Y Position"]],
      cell = mtx[["Cell ID"]]
    )
  } else if (mtx.file.type == "QuPath") {
    # no cell ID column in the QuPath output
    if ("Centroid X µm" %in% colnames(mtx)) {
      x.pos <- "Centroid X µm"
      y.pos <- "Centroid Y µm"
    } else {
      x.pos <- "Centroid X px"
    ...
    feature.cols <- colnames(mtx)[grepl("Cell: Mean", colnames(mtx), ignore.case=T)]
    feature.names <- lapply(feature.cols, FUN = function(x) strsplit(x, ": ")[[1]][1])
    outs[["matrix"]] <- t(mtx[, feature.cols])
    rownames(outs[["matrix"]]) <- feature.names
    colnames(outs[["matrix"]]) <- 1:nrow(mtx)
...
#' Load Vizgen data
#'
#' @param data.dir Path to folder containing Vizgen MERFISH files
#' @param fov Name of fov to store in fovs slot of Seurat object
#' @param z.slice Z slice to load; must be between 0 and 6, inclusive
...
#' @concept preprocessing
#'
LoadVizgen <- function(data.dir, fov = "fov", z.slice = 3L) {
  data <- ReadVizgen(
    data.dir = data.dir,
    filter = "^Blank-",
  ...
  coords <- CreateSpatialCoords(
    coords = segmentations.data,
    type = c("segmentation", "centroids"),
    molecules = data$microns
  )
  obj <- CreateSeuratObject(counts = data$transcripts)
  coords <- subset(coords, cells = intersect(Cells(coords[["segmentation"]]), Cells(obj))) # only consider the cells we have counts and a segmentation for. Cells which don't have a segmentation are probably found in other z slices.

  # add coords to seurat object
  obj[[fov]] <- coords
  # DefaultSegmentation(obj[[fov]]) <- "centroids"
  return(obj)
}

#' Load Nanostring SMI data
#'
#' Load Nanostring SMI data
#'
...
#' @concept preprocessing
#'
LoadNanostring <- function(data.dir, fov = "fov") {
  data <- ReadNanostring(
    data.dir = data.dir,
    metadata = c("Area", "fov", "Mean.MembraneStain", "Mean.DAPI", "Mean.G",
          "Mean.Y", "Mean.R", "Max.MembraneStain", "Max.DAPI", "Max.G",
          "Max.Y", "Max.R"),
    type=c("centroids", "segmentations")
  )
  ...
  coords <- subset(coords, cells = intersect(Cells(coords[["segmentation"]]), Cells(obj)))
  obj[[fov]] <- coords
  # DefaultSegmentation(obj[[fov]]) <- "centroids"
  return(obj)
}
...
#' Load Akoya CODEX data
#'
#' @param mtx Path to CSV generated by CODEX Processor version 1.7.0.6
#' @param type One of the following values depending on how data was preprocessed
#' \itemize{
#'  \item \dQuote{\code{processor}}: matrix generated by CODEX Processor
#'  \item \dQuote{\code{inForm}}: matrix generated by inForm
#'  \item \dQuote{\code{QuPath}}: matrix generated by QuPath
...
#' @concept preprocessing
#'
LoadAkoya <- function(mtx, type = "inForm", fov = "fov") {
  data <- ReadAkoya(mtx = mtx, file.type = type) # read in matrix and centroids
  coords <- CreateSpatialCoords(coords = data$centroids, type="centroids") # convert centroids into coords object
  obj <- CreateSeuratObject(counts = data$matrix) # build Seurat object from matrix
  coords <- subset(coords, cells = Cells(obj)) # make sure coords only contain cells in seurat object
  obj[[fov]] <- coords # add image to seurat object
  return(obj)
}

#' Load HuBMAP CODEX data
#'
#' Load HuBMAP CODEX data
#'
#' @param data.dir Path to a directory containing Vitessce cells and clusters JSONs
#' @param fov Name of fov to store in fovs slot of Seurat object
#' @return Seurat object
#'
#' @export
...
#' @concept preprocessing
#'
LoadHuBMAPCODEX <- function(data.dir, fov = "fov") {
  # TODO: come up with a better function names
  data <- ReadVitessce(
    counts = paste0(data.dir, "reg1_stitched_expressions.clusters.json"),
    coords = paste0(data.dir, "reg1_stitched_expressions.cells.json"),
    type = "segmentations"
  )
  ...
  obj <- CreateSeuratObject(data$counts)
  coords <- subset(coords, cells = Cells(obj)) # make sure spatial coords only contain cells in seurat object
  obj[[fov]] <- coords
  return(obj)
}
