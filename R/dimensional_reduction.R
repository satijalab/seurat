...
#' Create a graph connectivity object in coo format
#'
...
CreateCoo <- function(data = NULL){
  ...
  data.summary <- as.matrix(summary(data))
  data.summary <- data.summary[which(data.summary[,3]>0),]
  colnames(data.summary) <- c("from","to","value")
  
  coo.obj <- list(coo=data.summary, names=rownames(data), n.elements=nrow(data))
  class(coo.obj) <- "coo"
  return (coo.obj)
}


#' Run UMAP on k nearest neighbors
#'
...
#' @param adjacency.use A square adjacency matrix showing pairwise cell similarities
#' @param knn.use A list of length 2 showing the indices and distances between knn. Can be
#' outputs from running RANN::nn2, FNN::get.knn or similar functions
#' @param genes.for.knn a vector of genes for searching nearest neighbors
#' @param assay Name of assay to use if genes.for.knn is provided
#' @param reduction Dimensional reduction to use for searching nearest neighbors, used if no adjacencies, 
#' knn or genes.for.knn is specified. Default is pca.
#' @param dims Dimensions to use for reduction embeddings
#' @param k.param Number of k nearest neighbors to search
#' @param eps Error bound when performing nearest neighbor seach using RANN; 
#' default of 0.0 implies exact nearest neighbor search
#' @param min_dist min_dist: This controls how tightly the embedding is allowed compress points together. 
...
#' @param metric metric: This determines the choice of metric used to measure distance in the input space. A wide 
...
RunkUMAP <- function(object, adjacency.use = NULL, knn.use = NULL, genes.for.knn = NULL, assay = "RNA", 
  ...
  config$n_neighbors <- k.param
  if (sum(is.na(config[c("a","b")])) > 0){
    config[c("a", "b")] = umap::find.ab.params(config$spread, config$min_dist)
  }
  
  if (!is.null(adjacency.use)){
    coo.graph <- CreateCoo(data = adjacency.use)
  }
  else if (!is.null(knn.use)){
    if (!is(knn.use,"list")) stop("Input knn is not a list")
    if (!length(knn.use) == 2) stop("Input knn should have two elements: indices and distances")
    names(knn.use) <- c("indexes","distances")
    coo.graph <- umap::naive.fuzzy.simplicial.set(knn = knn.use,config = config)
  }
  else {
    if (!is.null(genes.for.knn)){
      knn.data <- t(GetAssayData(object = object,slot = "data",assay = assay))
    }
    else {
      knn.data <- object@reductions[[reduction]]@cell.embeddings[,dims]
    }
    
    knn.use <- nn2(data = knn.data,k = k.param,eps = eps)
    names(knn.use) <- c("indexes","distances")
    coo.graph <- umap::naive.fuzzy.simplicial.set(knn = knn.use,config = config)
  }
  
  embedding <- umap::make.initial.embedding(coo.graph$n.elements, config, coo.graph)
  embedding <- umap::naive.simplicial.set.embedding(coo.graph, embedding, config)
  colnames(embedding) <- c("kUMAP1","kUMAP2")
  rownames(embedding) <- colnames(object)
  
  kumap <- CreateDimReducObject(embeddings = embedding,key = "kUMAP",assay = "RNA")
  object[["kUMAP"]] <- kumap
