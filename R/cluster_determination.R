#' @include seurat.R
#' @include cluster_determination_generics.R
  ...
  for (r in resolution) {
    ...
    if (CalcInfoExists(object, paste0("FindClusters.res.", r)) & force.recalc != TRUE) {
      ...
      if (all(all.equal(old.parameters, parameters.to.store) == TRUE)) {
        warning(paste0(
          "Clustering parameters for resolution ",
...
FindClusters.loom <- function(
  ...
  if (is.null(x = graph.use)) {
    BuildSNN(
      ...
      save.SNN = save.SNN,
      nn.eps = nn.eps,
      overwrite = force.recalc
    )
    graph.name <- 'col_graphs/SNN'
  } else {
    graph.name <- graph.use
  }
  snn <- object$get.graph(name = graph.name)
  ...
  if (!save.SNN && is.null(x = graph.use)) {
    object$link_delete(name = graph.name)
  }
  object$flush()
  gc(verbose = FALSE)
  invisible(x = object)
