...
GetResidual<- function(
  ...
  if(is.null(Misc(object[[assay]])$vst.out)){
    stop( assay," assay is not generated by SCTransform.")
  }
  
  new_features<-setdiff(features, rownames(GetAssayData(object, assay = assay, slot ="scale.data" )))
  existed_features<-intersect(features, rownames(GetAssayData(object, assay = assay, slot ="scale.data" )))
  if ( length(new_features)==0){
    stop("Pearson residuals of ",paste(features,collapse=" ")," existed already.")
  }
  if ( length(existed_features)!=0){
    message("Pearson residuals of ",paste(existed_features,collapse=" ")," existed already.")
  }
 
  vst_out<-Misc(object[[assay]])$vst.out
  umi<- GetAssayData(object, assay = assay, slot ="counts" )[new_features,,drop=F]
