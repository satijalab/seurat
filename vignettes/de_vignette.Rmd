...
A key consideration for differential expression testing with single cell data is the unit of analysis. 
Analyses which treat individual cells as independent replicates do not account for biological variability, e.g. between different cells from different individuals or lines. 
Such pseudoreplicate-based analyses and have been shown to find a large number of false positive associations [Squair et al., 2021](https://www.nature.com/articles/s41467-021-25960-2), [Zimmerman et al., 2021](https://www.nature.com/articles/s41467-021-21038-1), [Junttila et al., 2022](https://academic.oup.com/bib/article/23/5/bbac286/6649780). Below, we show how pseudobulking can be used to account for such within-sample correlation. 
...
# Note that this test treats individual cells as the unit of analysis, which may be justified for comparing cell types within an individual. A significant p-value for a gene supports the conclusion that the gene has a different mean expression level in CD16 Mono vs. CD1 Mono cells from *this donor*. However, the findings should be interpreted cautiously since they do not necessarily support generalization of such differential expression to cells from *other donors*. See the section on pseudobulk analysis below.
...
### Perform DE analysis within the same cell type across conditions using pseudobulk expression

Since this dataset contains treatment information (control versus stimulated with interferon-beta), we can also ask what genes change in different conditions for cells of the same type. Pseudobulk gene expression profiles sum the counts from all cells from each biological sample (e.g. individual donor, cell line, or animal), which is the appropriate unit of analysis for statistical testing of differential expression between conditions. 
...
### Comparison with pseudoreplicate (single cell) DE analysis

Treating individual cells as the unit of analysis (i.e. pseudoreplication) can lead to low p-values due to the large number of cells in single cell studies, but does not account for variability between samples. Here we will explore the difference between pseudobulk and single-cell based DE analysis. First, we create a column in the `meta.data` slot to hold both the cell type and treatment information and switch the current `Idents` to that column. Then we use `FindMarkers()` to find the genes that are different between control and stimulated CD14 monocytes.
...
Here, SRGN and HLA-DRA both have very small p-values in the single-cell analysis (on the orders of $10^{-21}$ and $10^{-9}$), but much larger p-values around 0.18 in the pseudobulk analysis. While there appears to be a difference between control and stimulated cells when ignoring sample information, the signal is much weaker on the sample level, and we can see notable variability from sample to sample. 
...
* "MAST" : GLM-framework that treats cellular detection rate as a covariate (Finak et al, Genome Biology, 2015) ([Installation instructions](https://bioconductor.org/packages/release/bioc/html/MAST.html))
* "DESeq2" : DE based on a model using the negative binomial distribution (Love et al, Genome Biology, 2014) ([Installation instructions](https://bioconductor.org/packages/release/bioc/html/DESeq2.html))
