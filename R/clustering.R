...
#' @param fix.wnn.weights A vector of fixed weights (should have the same length as the reduction.list) used for WNN calculation.
#' It is a more convenient way to set the weights than generating a new 'ModalityWeights' object.
...
FindMultiModalNeighbors  <- function(
    ...
    if (is.null(x = modality.weight) & is.null(fix.wnn.weights)) {
        if (verbose) {
    ...
    } else if (is.null(x = modality.weight) & !is.null(fix.wnn.weights)){
        if (verbose) {
            message("Setting wnn weights using the provided fix.wnn.weights")
        }
        modality.weight <- FindModalityWeights(
            object = object,
            reduction.list = reduction.list,
...
# Calculate or fix modality weights
#
# This function calculates cell-specific modality weights which are used in
# WNN analysis. It can operate in two modes:
# 1. Calculate weights automatically (when fix.wnn.weights = NULL)
# 2. Use preset weights (when fix.wnn.weights is provided)
#
#' @inheritParams FindMultiModalNeighbors
# @param object A Seurat object
# @param fix.wnn.weights A list of preset wnn weights (same length as reduction.list).
#                        If NULL, weights will be calculated automatically.
...
FindModalityWeights <- function(
    ...
    use_fixed_weights <- !is.null(fix.wnn.weights)
    
    if (use_fixed_weights) {
        # Validate fixed weights input
        if (length(fix.wnn.weights) != length(reduction.list)) {
            stop("Please make sure the fix.wnn.weights is correctly specified! It should have the same length as your reduction.list.")
        }
        if (smooth) {
            warning("'smooth = TRUE' is not applicable when wnn.weights are pre-set. Ignoring...")
            smooth <- FALSE
        }
    }
    ...
    # Set names for all lists
    if (use_fixed_weights) {
        names(x = fix.wnn.weights) <- names(x = reduction.list) <- names(x = dims.list) <-
    ...
    if (verbose && use_fixed_weights) {
        message("Completed Finding ", k.nn, " nearest neighbors for each modality.")
        print(Sys.time())
    }
    ...
    # Initialize variables for calculated weights mode
    within_impute <- list()
    cross_impute <- list()
    within_impute_dist <- NULL
    cross_impute_dist <- NULL
    within_impute_kernel <- NULL
    cross_impute_kernel <- NULL
    
    if (!use_fixed_weights) {
        # Original calculation mode - calculate within and cross modality distances
    ...
    # Calculate kernel width (needed for both modes)
    if (snn.far.nn) {
        if (verbose) {
    ...
    # Calculate modality scores based on mode
    if (use_fixed_weights) {
        # Fixed weights mode - use preset weights
        modality_score <- lapply(
            X = reduction.list,
            FUN = function(idx) {
                score.r = rep(fix.wnn.weights[[idx]], nrow(embeddings.list.norm[[ reduction.list[[idx]] ]]))
                return(score.r)
            }
        )
        names(modality_score) = names(reduction.list)
        modality.weight <- modality_score
    } else {
        # Calculated weights mode - compute kernels and scores
