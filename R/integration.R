...
#' @param reference.neighbors Precomputed Neighbors for the reference, generated by NNHelper
...
FindMappingAnchors <- function(
  ...
  if (is.null(reference.neighbors)) {
    stop("reference.neighbors must be provided. Use NNHelper to generate the Neighbors object.")
  }
  projected = TRUE
  normalization.method <- match.arg(arg = normalization.method)
  ...
  if( ncol(x = Indices(reference.neighbors)) < (k.nn+1)){
  ...
  anchors <- FindAnchors(
    ...
    reduction = reduction,
    internal.neighbors = list(reference.neighbors, query.neighbors),
    dims = dims,
    k.anchor = k.anchor,
    k.filter = k.filter,
    k.score = k.score,
    max.features = max.features,
    nn.method = nn.method,
    nn.idx1 = Index(reference.neighbors),
...
#' @param nn.method Method for nearest neighbor finding. Options include: rann,
#' annoy
#' @param reference.index Optional precomputed Neighbors index object returned by NNHelper with cache.index = TRUE
