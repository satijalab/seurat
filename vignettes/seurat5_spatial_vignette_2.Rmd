...
# Loading segmentations is a slow process and multi processing with the future package is recommended
...
Using the positional information of each cell, we compute spatial niches. We use a cortex reference from the Allen Brain Institute to annotate cells, so we first crop the dataset to the cortex. The Allen Brain reference can be installed [here](https://www.dropbox.com/s/cuowvm4vrf65pvq/allen_cortex.rds?dl=1).
