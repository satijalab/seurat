...
brain[["Spatial"]] <- CreateAssay5Object(brain[["Spatial"]]@counts)
```
...
brain2 <- UpdateSeuratObject(brain2)
brain2[["Spatial"]] <- CreateAssay5Object(counts = brain2[["Spatial"]]@counts)
...
slide.seq <- UpdateSeuratObject(slide.seq)
slide.seq[["Spatial"]] <- CreateAssay5Object(counts = slide.seq[["Spatial"]]@counts)
```
...
Annotation using RCTD

```{r rctd}
library(spacexr)

# allen cortex reference
allen.cortex <- readRDS("/brahms/hartmana/seurat_site_builder/seurat5/seurat-private/data/allen_cortex.rds")
Idents(allen.cortex) <- "subclass"

# need to drop the cell types which there are <25 of
drop.cells <- WhichCells(allen.cortex, idents = "CR")
allen.cortex <- subset(allen.cortex, cells = drop.cells, invert = TRUE)

counts <- LayerData(allen.cortex, layer = "counts")
cluster <- as.factor(allen.cortex$subclass)
levels(cluster)[3] <- "L2 3 IT"
names(cluster) <- colnames(allen.cortex)
nUMI <- allen.cortex$nCount_RNA
names(nUMI) <- colnames(allen.cortex)
reference <- Reference(counts, cluster, nUMI)

counts <- LayerData(cortex, layer = "counts")
...
cortex <- AddMetaData(cortex, metadata = myRCTD@results$results_df)
