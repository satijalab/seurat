...
DownsampleMatrix <- function(mat, size, method = 'max', eps = 0.001, bw.adjust = 2, ...) {
  ...
  dens <- apply(X = mat, MARGIN = 2, FUN = function(y) {
    y.dens <- density(x = y, bw = 'nrd', adjust = bw.adjust)
    approx(x = y.dens$x, y = y.dens$y, xout = y)$y
  })
  ...
  weights <- sweep(x = weights, MARGIN = 2, STATS = apply(X = weights, MARGIN = 2, FUN = sum), FUN = '/')
