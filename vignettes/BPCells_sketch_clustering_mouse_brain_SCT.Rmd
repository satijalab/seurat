---
title: "Sketch clustering in mouse brain (SCTransform)"
...
obj <- RunUMAP(obj, dims = 1:50, return.model = T, verbose = F)
```
...
```
```{r}
# add some fake annotations to the object
obj$predicted.subclass <- sample(c("celltype1", "celltype2"), length(Cells(obj)), replace = TRUE)
obj$predicted.cluster <- sample(c("celltype1", "celltype2", "celltype3", "celltype4"), length(Cells(obj)), replace = TRUE)
```
