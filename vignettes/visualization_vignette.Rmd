...
Here, we demonstrate visualization techniques in Seurat using our previously computed Seurat object from the 2,700 PBMC guided clustering tutorial. You can access the dataset and load it into a Seurat object using [SeuratData](https://github.com/satijalab/seurat-data).
...
# DimPlot replaces TSNEPlot, PCAPlot, etc., and plots either "umap", "tsne", or "pca" by default, in that order
...
```{r themeing.2, fig.height=6}
baseplot + FontSize(x.title = 20, y.title = 20) + NoLegend()
```

```{r themeing.3, fig.height=6}
...
select.cells <- readLines(con = '/brahms/shared/vignette-data/pbmc_dcs_cells.txt')
...
In early Seurat versions, we used the `CombinePlots()` function to combine multiple plots together. 

All plotting functions now use the `patchwork` system, which allows for more flexible plot arrangements and easier customization of combined plots; see the [patchwork website](https://patchwork.data-imaginist.com/) for more details and examples. 
