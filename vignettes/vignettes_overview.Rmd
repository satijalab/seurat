...
If you are looking to analyze multi-modal data, we provide an analysis example to familiarize users with the Seurat infrastructure necessary for working with multiple modalities. This example features CITE-seq (RNA + protein) data but the general principles are extendable to arbitrary modalities. Similarly, for spatially-resolved data we work through a few examples of common analysis tasks such as interactive visualization and identifying spatially variable patterns of gene expression. Finally, Seurat offers an extensive suite functionality for visually exploring your single-cell data. Check out some of the possibilities in the vignette below.
