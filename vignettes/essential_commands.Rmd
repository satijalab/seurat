...
Here, we present some essential commands for working with Seurat objects. See also the v5-specific [essential commands](seurat5_essential_commands.html) list.
...
# install datasets from SeuratData
InstallData('pbmc3k')
InstallData('cbmc')
InstallData('ifnb')
...
See the introductory [integration vignette](integration_introduction.html) for more information.
...
See the [DE vignette](de_vignette.html) for information  on how to add the `donor_id` column to meta data.
...
Seurat has a vast, ggplot2-based plotting library. All plotting functions will return a ggplot2 plot by default, allowing easy customization.
...
```{r newthings}
# Available in v5.0+!
# Note that plotting functions return ggplot2 objects, so you can add themes, titles, and options onto them
...
* [SeuratObject documentation](https://satijalab.github.io/seurat-object/index.html)
* [Seurat v5+-specific essential commands](seurat5_essential_commands.html)
