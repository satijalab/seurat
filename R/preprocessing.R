...
GetResidual <- function(
  ...
  if (length(x = object[[assay]]@SCTModel.list) == 0) {
    warning("SCT model not present in assay")
    return(object)
  }
  # for object created in 3.1.1 or earlier, default to RNA
  ...
  if (length(x = new_features) == 0) {
    ...
    if (length(object[[assay]]@SCTModel.list) == 1) {
      new.scale.data <- GetResidualSCTModel(
        object = object,
        assay = assay,
        SCTModel  = object[[assay]]@SCTModel.list[[1]],
        new_features = new_features,
        verbose = verbose
      )
    } else {
  
      new.scale.data.list <- lapply(X = object[[assay]]@SCTModel.list, 
                                    FUN = function(model){
                                      residual.m <- GetResidualSCTModel(
                                        object = object,
                                        assay = assay,
                                        SCTModel = model,
                                        new_features = new_features,
...
SCTransform <- function(
  ...
  if (conserve.memory & is.null(x = reference.SCT.model)) {
    ...
    if (!is.null(x = residual.features)) {
      ...
      if (!is.null(x = reference.SCT.model)) {
        # center residuals by reference residual mean
...
GetResidualSCTModel <- function(
  ...
  if ( length(x = diff_features)  == 0) {
    ...
    if (length(x = intersect_feature) == 0) {
      new_residual <- matrix(data = 0, nrow = length(new_features), ncol = length(Cells.SCTModel(SCTModel)))
      colnames(new_residual) <- Cells.SCTModel(SCTModel)
      rownames(new_residual) <- new_features
      return(new_residual)
    }
    umi <- GetAssayData(object = object, assay = umi.assay, slot = "counts" )[intersect_feature, Cells.SCTModel(SCTModel) , drop = FALSE]
  }
  ...
  vst_out <- SCTModel_to_vst(SCTModel = SCTModel)
...
SCTModel_to_vst <- function( SCTModel, feature.params = NULL ){
  ...
  vst_out <- list()
  vst_out$model_str <- SCTModel@model
  vst_out$model_pars_fit <- as.matrix(SCTModel@feature.attributes[, feature.params])
  vst_out$cell_attr <- SCTModel@cell.attributes
  return(vst_out)
}
