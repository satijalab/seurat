...
GetResidual <- function(object, features, assay = "SCT", verbose = TRUE) {
  ...
  if (is.null(object[[assay]]@misc$vst.out)) {
    stop( paste(assay,"assay is not generated by SCTransform.", sep = " "))
  }
  new_features <- setdiff(x = features, y = rownames(x = object[["SCT"]]@scale.data))
  existed_features <- intersect(x = features, y = rownames(x = object[["SCT"]]@scale.data))
  ...
  umi <- object[["SCT"]]@counts[new_features, , drop = FALSE]
