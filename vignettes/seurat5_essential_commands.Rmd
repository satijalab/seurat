...
```{r}
assay.v3 <- CreateAssayObject(counts = pbmc.data)
assay.v5 <- CreateAssay5Object(counts = pbmc.data)
...
Assay data can be accessed with `FetchData` depending on the use case.

```{r}
# returns information from both assay and meta.data as a data.frame
fetch_df <- FetchData(object = pbmc, layer = "counts", vars = c("rna_MS4A1", "rna_CD79A", 'split.data'))
head(fetch_df)
```
