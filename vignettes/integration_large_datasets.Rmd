---
title: "Large Dataset Integration"
output:
  html_document:
    theme: united
    df_print: kable
...
For very large datasets, the standard integration workflow can sometimes be prohibitively computationally expensive. In this workflow, we discuss alternatives that can improve efficiency and runtimes, namely RPCA and reference-based integration.
...
Next, select features for downstream integration, and run PCA on each object in the list, which is required for running the alternative reciprocal PCA workflow, described in more detailed below.
