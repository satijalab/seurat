...
The Assay5 object is able to support different matrix data structures to best suit particular analyses. The PBMC 3k dataset contains ~3,000 cells, which can easily be stored in-memory as a sparse `dgCMatrix`. On the other hand, it can be impractical in terms of memory and computation to store and concomitantly analyze millions of cells in-memory. Thus, the mouse brain dataset stores counts on-disk in an Assay5 object. The mouse brain 1 million cell object is constructed as described in the [BPCells vignette](seurat5_bpcells_interaction_vignette.html).
...
head(pbmc3k[["RNA"]]$counts[, Cells(pbmc3k)[1:20]])
head(pbmc3k[["RNA"]][["counts"]][, Cells(pbmc3k)[1:20]])
