...
eccite[["RNA"]] <- as(eccite[["RNA"]], Class = "Assay5")
eccite[["ADT"]] <- as(eccite[["ADT"]], Class = "Assay5")
eccite[["HTO"]] <- as(eccite[["HTO"]], Class = "Assay5")
eccite[["GDO"]] <- as(eccite[["GDO"]], Class = "Assay5")
...
eccite <- ScaleData(object = eccite, do.scale = FALSE, do.center = TRUE)
...
q1 <- DimPlot(
  ...
  group.by = "replicate",
  reduction = "prtbumap",
  pt.size = 0.2, cols = "Dark2", label = FALSE, repel = TRUE) +
  scale_color_brewer(palette = "Dark2") +
  ggtitle("Biological Replicate") +
  ylab("UMAP 2") +
  xlab("UMAP 1")
...
write.csv(x = t(as.data.frame(all_times)), file = "../output/timings/seurat5_mixscape_vignette_times.csv")
```
