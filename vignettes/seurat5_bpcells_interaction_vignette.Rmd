...
### Sharing objects with on-disk layers

`SaveSeuratRds()` records the *path* of each on-disk matrix, so the `.Rds` alone is not self-contained: if the object is emailed, uploaded, or the matrix directory is moved or renamed, the layers can no longer be read.

`SaveSeurat()` bundles the object together with all of its on-disk matrix stores into a single archive, and `LoadSeurat()` extracts it and reconnects the layers automatically. The bundle can be copied or shared like any other file.
...
If the data fits in memory, `AsInMemory()` instead materializes the on-disk layers, so a plain `saveRDS()` yields a single portable file with no external stores to carry along.
