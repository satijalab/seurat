...
test_that("features parameter behaves correctly ", {
  ...
  expect_equal(vargenes.results[20, "avg_logFC"], 1.597696, tolerance = 1e-6)
...
test_that("logfc.threshold works", {
  expect_equal(nrow(x = results), 118)
...
test_that("latent.vars works", {
  ...
  expect_equal(results[1, "avg_logFC"], -3.08215, tolerance = 1e-6)
...
test_that("group.by works", {
  expect_equal(nrow(x = results), 188)
  expect_equal(results, results2)
  expect_equal(results[1, "p_val"], 0.02870319)
  expect_equal(results[1, "avg_logFC"], 0.822672, tolerance = 1e-6)
...
test_that("subset.ident works", {
  expect_equal(nrow(x = results), 182)
  expect_equal(results, results2)
  expect_equal(results[1, "p_val"], 0.01293720)
  expect_equal(results[1, "avg_logFC"], 1.79928, tolerance = 1e-6)
...
test_that("FindMarkers with wilcox_limma works", {
  ...
  expect_equal(nrow(x = markers.0.limma), 227)
  ...
  expect_equal(nrow(x = results.clr.limma), 167)
...
if (requireNamespace('metap', quietly = TRUE)) {
  ...
  test_that("FindConservedMarkers handles missing idents in certain groups", {
    ...
    expect_equal(nrow(markers.missing), 225)
