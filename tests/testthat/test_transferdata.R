...
test_that("TransferData default work", {
  ...
  expect_equal(preds.standard[1, 1], "0")
  expect_equal(preds.standard[1, 5], 0.3916828, tolerance = 1e-6)
  ...
  expect_equal(sum(GetAssayData(pred.assay)[1, ]), 66.80004, tolerance = 1e-6)
  expect_equal(sum(GetAssayData(pred.assay)[, 1]), 260.2376, tolerance = 1e-6)
...
test_that("TransferData can return predictions assay, ", {
  ...
  expect_equal(pred.assay@var.features, logical(0))
  expect_equal(ncol(pred.assay@meta.features), 0)
  expect_equal(sum(GetAssayData(pred.assay)[1, ]), 27.47347, tolerance = 1e-6)
  expect_equal(sum(GetAssayData(pred.assay)[, 1]), 1.391683, tolerance = 1e-6)
