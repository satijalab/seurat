...
nbt=setup(nbt,project="Seurat",min.cells = 3,names.field = 2,names.delim = "_",min.genes = 1000,calc.noise=FALSE,is.expr=1) 
```
...
nbt=project.pca(nbt,do.print = TRUE,pcs.print = 3,genes.print=5)
nbt.sig.genes=pca.sig.genes(nbt,1:9,pval.cut = 1e-5,max.per.pc = 500)
...
Cluster NBT data
```{r}
#run tSNE with default settings (assume perplexity=30, note that perplexity is the number of 'expected' neighbor cells)
nbt=run_tsne(nbt,dims.use=1:10,max_iter=1000,reduction.use = "pca")
tsne.plot(nbt)


#an alternative (faster) workflow. First run C++ approx, then refine 
nbt=run_tsne(nbt,pcs.use = 1:8,do.fast=TRUE)
nbt=add_tsne(nbt,pcs.use=1:8,add.iter = 100)
pca.plot(nbt,1,2,reduction.use = "tsne")
...
nbt2=Mclust_dimension(nbt,1,2,reduction.use = "tsne",G.use = 1.8,set.ident = TRUE)
tsne.plot(nbt2)
...
nbt=buildClusterTree(nbt,do.reorder = TRUE,reorder.numeric = TRUE,genes.use = nbt.sig.genes)
```

Do some biology!

```{r, fig.width = 9, fig.height = 9}
#Find that iPS cells are in cluster 7
ips.cells=which.cells(nbt,"iPS",id = "orig")
fetch.data(nbt,"m",ips.cells)
...
#thresh.use speeds things up (increase value to increase speed) but might miss more subtle differences 
#Note that Seurat finds both positive and negative markers (myDiff either >0 or <0)
ips.markers=find.markers(nbt,7,thresh.use = 4,test.use = "roc")
print(head(ips.markers,10))

#note that Seurat has four marker tests:
#ROC test ("roc"), t-test ("t"), LRT test based on zero-inflated data ("bimod"), LRT test based on tobit-censoring models ("tobit")
ips.markers=find.markers(nbt,7,thresh.use = 4,test.use = "t")
...
neuro.markers=find.markers(nbt,1,2,thresh.use = 3,test.use = "roc")
print(head(neuro.markers,20))



#visualize markers in a few different ways
feature.plot(nbt,c("BHLHE22","NEUROD2","GLI3","VIM"),pt.size = 1)
feature.heatmap(nbt,c("BHLHE22","NEUROD2","GLI3","VIM"),pt.size = 1.5)
vlnPlot(nbt,c("BHLHE22","NEUROD2","GLI3","VIM"))

#switch the identity class definition back to the original cell ID ('orig')
nbt=set.all.ident(nbt,"orig")

#note that these markers are highly variable based on the original groupings
vlnPlot(nbt,c("SOX9","BHLHE22"))
...
plot(nbt.avg[,"c8"],nbt.avg[,"c7"],pch=16,cex=0.8)

# uncomment the next line to identify individual genes
#identify(nbt.avg[,"c8"],nbt.avg[,"c7"],labels = rownames(nbt.avg))
