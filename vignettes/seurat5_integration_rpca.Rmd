...
# load dataset
data('ifnb') # what if I do data here instead
...
#features <- SelectIntegrationFeatures(ifnb.list) # this is in the vignette
features <- VariableFeatures(ifnb)

ifnb <- ScaleData(ifnb, features = features, verbose = FALSE)
ifnb <- RunPCA(ifnb, features = features, verbose = FALSE)
ifnb
...
#ifnb <- IntegrateData(anchorset = immune.anchors)
...
library(ggplot2)
plot <- DimPlot(ifnb, group.by = "stim") +
...
ifnb <- SCTransform(ifnb, method = "glmGamPoi")
features <- VariableFeatures(ifnb)
ifnb <- RunPCA(ifnb, features = features)
```
...
#write.csv(x = t(as.data.frame(all_times)), file = "../output/timings/seurat5_integration_rpca.csv")
```
