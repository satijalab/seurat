...
setMethod("find.clusters", signature = "seurat",
          function(object, genes.use=NULL, pc.use=NULL, SNN = NULL, k_param=10, k_scale =10, plot.SNN=FALSE,prune.SNN=0.1, save.SNN = FALSE,
            ...
            if (do_modularity){
              if(do_sparse){
                object=doModularity_Clust(object, SNN_sp, modularity, resolution, algorithm, n_start, n_iter, random_seed, print_output, ModularityJarFile)
                object=groupSingletons(object,SNN_sp)
              }
              else{
...
setMethod("validate.clusters", signature = "seurat", function(object, pc.use=NULL, top.genes=30, SNN = NULL, min_connectivity = 0.01, acc.cutoff=0.9, verbose = TRUE){
    ...
    if(verbose) print(paste("100% complete --- started with ", num_clusters_orig," clusters, ", length(unique(object@ident)), " clusters remaining", sep = "" ))
    return(object)
  }
)
