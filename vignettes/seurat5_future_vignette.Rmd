...
To access the parallel version of functions in Seurat, you need to load the `future` package and set the `plan`. The `plan` will specify how the function is executed. The default behavior is to evaluate in a non-parallelized fashion (sequentially). To achieve parallel (asynchronous) behavior, we typically recommend the "multisession" strategy. By default, this uses all available cores but you can set the `workers` parameter to limit the number of concurrently active futures. 
...
`future` offers two asynchronous strategies relevant to Seurat. Pick based on your platform:

| Strategy | Platforms | Mechanism | Notes |
|----------|-----------|-----------|-------|
| `multicore` | Unix, macOS, and Linux **containers** only — **not** Windows, and not inside RStudio | Forks the current process; workers share memory, so nothing is serialized | Fast; the recommended path where available |
| `multisession` | All platforms, including Windows | Launches background R sessions; globals are serialized to workers over sockets | Portable but much slower; large objects can exceed `future.globals.maxSize`, and complex objects may hang or fail to serialize |

> **Note:** the `"multiprocess"` strategy was deprecated and removed from `future` (>= 1.20.0). It no longer exists — use `multicore` or `multisession` explicitly.

> **Windows users (including WSL):** `multicore` cannot fork on Windows. You can either accept `multisession`'s limitations, or run Seurat inside the Linux container provided by the repository's [`Dockerfile`](../Dockerfile), where `plan("multicore")` works. The `docker-multicore` CI job smoke-tests exactly this path.
...
pbmc[["RNA"]] <- as(pbmc[["RNA"]], Class = "Assay5")
...
timing.comparisons <- rbind(timing.comparisons, data.frame(fxn = "ScaleData", time = as.numeric(end - start, units = "secs"), strategy = "multisession"))

start <- Sys.time()
markers <- FindMarkers(pbmc, ident.1 = "NK", verbose = FALSE)
end <- Sys.time()
timing.comparisons <- rbind(timing.comparisons, data.frame(fxn = "FindMarkers", time = as.numeric(end - start, units = "secs"), strategy = "multisession"))
```
...
write.csv(x = t(as.data.frame(all_times)), file = "../output/timings/seurat5_future_vignette_times.csv")
```
