...
pcHeatmap(nbt,pc.use = 1,use.full = TRUE,do.balanced = TRUE)
...
```{r,fig.height=5,fig.width=7,cache=TRUE}
# As a demonstration, we first run the tSNE using the input of our significant gene set, and get poor separation
nbt=run_tsne(nbt,genes.use = nbt.sig.genes,max_iter=2000)
tsne.plot(nbt,pt.size = 1)

# Instead if we run tSNE using our 11 significant PCs as input (spectral tSNE), we get distinct point clouds 
