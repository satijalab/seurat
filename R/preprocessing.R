...
GetResidual <- function(
  ...
  if (is.null(x = Misc(object[[assay]], slot = 'vst.out')) & is.null(x = Misc(object[[assay]], slot = 'vst.set')) ) {
  ...
  if (length(x = new_features) == 0) {
    ...
    # Calculate Pearson Residule from integrated object SCT assay
    vst.set <- Misc(object[[assay]], slot = 'vst.set')
    scale.data <- GetAssayData(object = object, 
                               assay = assay, 
                               slot = "scale.data")
    object <- SetAssayData(
      object = object, 
    ...
    for (v in 1:length(vst.set)){
      ...
      object.v <- GetResidual_VstOut(object.v, 
                                     ...
                                     clip.range, 
                                     verbose)
      new.scale.data <- cbind(new.scale.data, 
                              GetAssayData(object.v,  assay = assay, slot ="scale.data" )[new_features,, drop = FALSE]
                              )
      }
    object <- SetAssayData(
      object = object, 
...
GetResidual_VstOut <- function(object, 
  ...
  if (length(x = diff_features) == 0) {
    ...
    warning("The following ", length(x = diff_features), " features do not exist in the counts slot: ", paste(diff_features, collapse = " "))
