...
test_that("AverageExpression works for different slots", {
  ...
  suppressWarnings(avg.counts <- AverageExpression(object, slot = 'counts')$RNA)
  expect_equal(
    unname(avg.counts['MS4A1', ]),
    unname(c(a = 0.37037037, b = 0.3461538, c = 0.3333333)),
    tolerance = 1e-6
  )
  expect_equal(
    unname(avg.counts['SPON2', ]),
    unname(c(a = 0.5185185, b = 0.6153846, c = 0.08333333)),
    tolerance = 1e-6
  )
...
test_that("AverageExpression with return.seurat", {
  ...
  expect_equal(
    ...
    unname(c(a = 0.31508105, b = 0.2972515, c = 0.2876821)),
    tolerance = 1e-6
  )
  expect_equal(
    unname(avg.data['SPON2', ]),
    unname(c(a = 0.4177352, b = 0.4795731, c = 0.08004271)),
    tolerance = 1e-6
  )
...
test_that("AverageExpression with multiple assays", {
  avg.test <- AverageExpression(object = object, assays = "TEST", slot = "data")
  ...
  avg.all <- AverageExpression(object = object, slot = "data")
