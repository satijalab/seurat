...
In the [standard Seurat workflow](https://satijalab.org/seurat/pbmc3k_tutorial.html) we focus on 10 PCs for this dataset, though we highlight that the results are similar with higher settings for this parameter. Interestingly, we've found that when using sctransform, we often benefit by pushing this parameter even higher. We believe this is because the sctransform workflow performs more effective normalization, strongly removing technical effects from the data. 

Even after standard log-normalization, variation in sequencing depth is still a confounding factor (see [Figure 1](https://www.biorxiv.org/content/biorxiv/early/2019/03/18/576827.full.pdf)), and this effect can subtly influence higher PCs. In sctransform, this effect is substantially mitigated (see [Figure 3](https://www.biorxiv.org/content/biorxiv/early/2019/03/18/576827.full.pdf)). This means that higher PCs are more likely to represent subtle, but biologically relevant, sources of heterogeneity -- so including them may improve downstream analysis.
...
As described in our [preprint](https://www.biorxiv.org/content/biorxiv/early/2019/03/18/576827.full.pdf), sctransform calculates a model of technical noise in scRNA-seq data using 'regularized negative binomial regression'. The residuals for this model are normalized values, and can be positive or negative. Positive residuals for a given gene in a given cell indicate that we observed more UMIs than expected given the gene’s average expression in the population and cellular sequencing depth, while negative residuals indicate the converse. 

The results of sctransfrom are stored in the "SCT" assay. You can learn more about multi-assay data and commands in Seurat in our [vignette](http://satijalab01.nygenome.org/seurat/v3.0/multimodal_vignette.html), [command cheat sheet](http://satijalab01.nygenome.org/seurat/essential_commands.html#multi-assay-features), or [developer guide](https://github.com/satijalab/seurat/wiki/Assay).
